...
unsigned char rijndael_inv_s[256] = {
...
unsigned char rijndael_rcon[256] = {
...
unsigned char rijndael_mult_2[256] = {
...
unsigned char rijndael_mult_3[256] = {
...
unsigned char rijndael_mult_9[256] = {
...
unsigned char rijndael_mult_11[256] = {
...
unsigned char rijndael_mult_13[256] = {
...
unsigned char rijndael_mult_14[256] = {
...
unsigned char* rijndael_round_keys(unsigned char* key) {
	unsigned char* keys = malloc(11 * 16);
	memcpy(keys, key, 16);
	for(size_t rcon = 1; rcon <= 10; rcon++) {
		unsigned char temp[4];
		memcpy(temp, keys + 16 * rcon - 4, 4);
		unsigned char rotate_temp = temp[0];
		temp[0] = rijndael_s[temp[1]] ^ rijndael_rcon[rcon];
		temp[1] = rijndael_s[temp[2]];
		temp[2] = rijndael_s[temp[3]];
		temp[3] = rijndael_s[rotate_temp];
		memxor(keys + 16 * rcon, keys + 16 * (rcon - 1), temp, 4);
		for(size_t idx = 4; idx < 16; idx++) {
			keys[16 * rcon + idx] = keys[16 * (rcon - 1) + idx] ^ keys[16 * rcon + idx - 4];
		}
	}
	return keys;
}

void rijndael_add_round_key(unsigned char* block, unsigned char* key) {
	memxor(block, block, key, 16);
}

void rijndael_sub_bytes(unsigned char* block) {
	for(size_t idx = 0; idx < 16; idx++) {
		block[idx] = rijndael_s[block[idx]];
	}
}

void rijndael_inv_sub_bytes(unsigned char* block) {
	for(size_t idx = 0; idx < 16; idx++) {
		block[idx] = rijndael_inv_s[block[idx]];
	}
}

void rijndael_shift_rows(unsigned char* block) {
...
void rijndael_inv_shift_rows(unsigned char* block) {
	unsigned char temp[3];
	temp[0] = block[13];
	block[13] = block[9];
	block[9] = block[5];
...
void rijndael_mix_columns(unsigned char* block) {
	for(size_t idx = 0; idx < 16; idx += 4) {
		unsigned char column[4];
		column[0] = rijndael_mult_2[block[idx]] ^ rijndael_mult_3[block[idx + 1]] ^ block[idx + 2] ^ block[idx + 3];
		column[1] = block[idx] ^ rijndael_mult_2[block[idx + 1]] ^ rijndael_mult_3[block[idx + 2]] ^ block[idx + 3];
		column[2] = block[idx] ^ block[idx + 1] ^ rijndael_mult_2[block[idx + 2]] ^ rijndael_mult_3[block[idx + 3]];
		column[3] = rijndael_mult_3[block[idx]] ^ block[idx + 1] ^ block[idx + 2] ^ rijndael_mult_2[block[idx + 3]];
		block[idx] = column[0];
		block[idx + 1] = column[1];
		block[idx + 2] = column[2];
		block[idx + 3] = column[3];
	}
}

void rijndael_inv_mix_columns(unsigned char* block) {
	for(size_t idx = 0; idx < 16; idx += 4) {
		unsigned char column[4];
		column[0] = rijndael_mult_14[block[idx]] ^ rijndael_mult_11[block[idx + 1]] ^ rijndael_mult_13[block[idx + 2]] ^ rijndael_mult_9[block[idx + 3]];
		column[1] = rijndael_mult_9[block[idx]] ^ rijndael_mult_14[block[idx + 1]] ^ rijndael_mult_11[block[idx + 2]] ^ rijndael_mult_13[block[idx + 3]];
		column[2] = rijndael_mult_13[block[idx]] ^ rijndael_mult_9[block[idx + 1]] ^ rijndael_mult_14[block[idx + 2]] ^ rijndael_mult_11[block[idx + 3]];
		column[3] = rijndael_mult_11[block[idx]] ^ rijndael_mult_13[block[idx + 1]] ^ rijndael_mult_9[block[idx + 2]] ^ rijndael_mult_14[block[idx + 3]];
		block[idx] = column[0];
		block[idx + 1] = column[1];
		block[idx + 2] = column[2];
		block[idx + 3] = column[3];
...
void rijndael_forward(unsigned char* block, unsigned char* key) {
	...
	for(size_t round = 1; round < 10; round++) {
		rijndael_sub_bytes(block);
		rijndael_shift_rows(block);
		rijndael_mix_columns(block);
		rijndael_add_round_key(block, round_keys + 16 * round);
	}
	rijndael_sub_bytes(block);
	rijndael_shift_rows(block);
...
void rijndael_reverse(unsigned char* block, unsigned char* key) {
	...
	for(size_t round = 9; round > 0; round--) {
		rijndael_inv_shift_rows(block);
		rijndael_inv_sub_bytes(block);
		rijndael_add_round_key(block, round_keys + 16 * round);
		rijndael_inv_mix_columns(block);
	}
	rijndael_inv_shift_rows(block);
	rijndael_inv_sub_bytes(block);
	rijndael_add_round_key(block, round_keys);
