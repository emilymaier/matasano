#include <stdlib.h>
#include <string.h>

#include "util.h"

static unsigned char s[256] = {
	
};

static unsigned char inv_s[256] = {
	
};

static unsigned char rcon[256] = {
	
};

static unsigned char mult_2[256] = {
	
};

static unsigned char mult_3[256] = {
	
};

static unsigned char mult_9[256] = {
	
};

static unsigned char mult_11[256] = {
	
};

static unsigned char mult_13[256] = {
	
};

static unsigned char mult_14[256] = {
	
};

static unsigned char* gen_round_keys(unsigned char* key) {
	unsigned char* keys = malloc(11 * 16);
	memcpy(keys, key, 16);
	for(size_t cur_rcon = 1; cur_rcon <= 10; cur_rcon++) {
		unsigned char temp[4];
		memcpy(temp, keys + 16 * cur_rcon - 4, 4);
		unsigned char rotate_temp = temp[0];
		temp[0] = s[temp[1]] ^ rcon[cur_rcon];
		temp[1] = s[temp[2]];
		temp[2] = s[temp[3]];
		temp[3] = s[rotate_temp];
		memxor(keys + 16 * cur_rcon, keys + 16 * (cur_rcon - 1), temp, 4);
		for(size_t idx = 4; idx < 16; idx++) {
			keys[16 * cur_rcon + idx] = keys[16 * (cur_rcon - 1) + idx] ^ keys[16 * cur_rcon + idx - 4];
		}
	}
	return keys;
}

static void add_round_key(unsigned char* block, unsigned char* key) {
	memxor(block, block, key, 16);
}

static void sub_bytes(unsigned char* block) {
	for(size_t idx = 0; idx < 16; idx++) {
		block[idx] = s[block[idx]];
	}
}

static void inv_sub_bytes(unsigned char* block) {
	for(size_t idx = 0; idx < 16; idx++) {
		block[idx] = inv_s[block[idx]];
	}
}

static void shift_rows(unsigned char* block) {
	unsigned char temp[3];
	temp[0] = block[1];
	block[1] = block[5];
	block[5] = block[9];
	block[9] = block[13];
	block[13] = temp[0];
	temp[0] = block[2];
	temp[1] = block[6];
	block[2] = block[10];
	block[6] = block[14];
	block[10] = temp[0];
	block[14] = temp[1];
	temp[0] = block[3];
	temp[1] = block[7];
	temp[2] = block[11];
	block[3] = block[15];
	block[7] = temp[0];
	block[11] = temp[1];
	block[15] = temp[2];
}

static void inv_shift_rows(unsigned char* block) {
	unsigned char temp[3];
	temp[0] = block[13];
	block[13] = block[9];
	block[9] = block[5];
	block[5] = block[1];
	block[1] = temp[0];
	temp[0] = block[14];
	temp[1] = block[10];
	block[14] = block[6];
	block[10] = block[2];
	block[6] = temp[0];
	block[2] = temp[1];
	temp[0] = block[15];
	temp[1] = block[11];
	temp[2] = block[7];
	block[15] = block[3];
	block[11] = temp[0];
	block[7] = temp[1];
	block[3] = temp[2];
}

static void mix_columns(unsigned char* block) {
	for(size_t idx = 0; idx < 16; idx += 4) {
		unsigned char column[4];
		column[0] = mult_2[block[idx]] ^ mult_3[block[idx + 1]] ^ block[idx + 2] ^ block[idx + 3];
		column[1] = block[idx] ^ mult_2[block[idx + 1]] ^ mult_3[block[idx + 2]] ^ block[idx + 3];
		column[2] = block[idx] ^ block[idx + 1] ^ mult_2[block[idx + 2]] ^ mult_3[block[idx + 3]];
		column[3] = mult_3[block[idx]] ^ block[idx + 1] ^ block[idx + 2] ^ mult_2[block[idx + 3]];
		block[idx] = column[0];
		block[idx + 1] = column[1];
		block[idx + 2] = column[2];
		block[idx + 3] = column[3];
	}
}

static void inv_mix_columns(unsigned char* block) {
	for(size_t idx = 0; idx < 16; idx += 4) {
		unsigned char column[4];
		column[0] = mult_14[block[idx]] ^ mult_11[block[idx + 1]] ^ mult_13[block[idx + 2]] ^ mult_9[block[idx + 3]];
		column[1] = mult_9[block[idx]] ^ mult_14[block[idx + 1]] ^ mult_11[block[idx + 2]] ^ mult_13[block[idx + 3]];
		column[2] = mult_13[block[idx]] ^ mult_9[block[idx + 1]] ^ mult_14[block[idx + 2]] ^ mult_11[block[idx + 3]];
		column[3] = mult_11[block[idx]] ^ mult_13[block[idx + 1]] ^ mult_9[block[idx + 2]] ^ mult_14[block[idx + 3]];
		block[idx] = column[0];
		block[idx + 1] = column[1];
		block[idx + 2] = column[2];
		block[idx + 3] = column[3];
	}
}

static void encipher(unsigned char* block, unsigned char* key) {
	unsigned char* round_keys = gen_round_keys(key);
	add_round_key(block, round_keys);
	for(size_t round = 1; round < 10; round++) {
		sub_bytes(block);
		shift_rows(block);
		mix_columns(block);
		add_round_key(block, round_keys + 16 * round);
	}
	sub_bytes(block);
	shift_rows(block);
	add_round_key(block, round_keys + 16 * 10);
}

static void decipher(unsigned char* block, unsigned char* key) {
	unsigned char* round_keys = gen_round_keys(key);
	add_round_key(block, round_keys + 16 * 10);
	for(size_t round = 9; round > 0; round--) {
		inv_shift_rows(block);
		inv_sub_bytes(block);
		add_round_key(block, round_keys + 16 * round);
		inv_mix_columns(block);
	}
	inv_shift_rows(block);
	inv_sub_bytes(block);
	add_round_key(block, round_keys);
}

static void add_padding(unsigned char* data, size_t size) {
	if(size % 16 != 0) {
		for(size_t pad_start = size; pad_start < (size / 16 + 1) * 16; pad_start++) {
			data[pad_start] = 16 - (size % 16);
		}
	}
}

static void remove_padding(unsigned char* data, size_t size) {
	if(size % 16 != 0) {
		data[size] = '\0';
	}
}

void rijndael_encrypt_ecb(unsigned char* data, size_t size, unsigned char* key) {
	add_padding(data, size);
	for(size_t start = 0; start < size; start += 16) {
		encipher(data + start, key);
	}
}

void rijndael_decrypt_ecb(unsigned char* data, size_t size, unsigned char* key) {
	for(size_t start = 0; start < size; start += 16) {
		decipher(data + start, key);
	}
	remove_padding(data, size);
}

void rijndael_encrypt_cbc(unsigned char* data, size_t size, unsigned char* key, unsigned char* iv) {
	add_padding(data, size);
	unsigned char prev_block[16];
	memcpy(prev_block, iv, 16);
	for(size_t start = 0; start < size; start += 16) {
		memxor(data + start, data + start, prev_block, 16);
		encipher(data + start, key);
		memcpy(prev_block, data + start, 16);
	}
}

void rijndael_decrypt_cbc(unsigned char* data, size_t size, unsigned char* key, unsigned char* iv) {
	unsigned char prev_block[16];
	memcpy(prev_block, iv, 16);
	for(size_t start = 0; start < size; start += 16) {
		unsigned char cur_block[16];
		memcpy(cur_block, data + start, 16);
		decipher(data + start, key);
		memxor(data + start, data + start, prev_block, 16);
		memcpy(prev_block, cur_block, 16);
	}
	remove_padding(data, size);
}
